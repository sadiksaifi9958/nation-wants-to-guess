function Score({ score, highScore, timer, selected }) {
    return (
        <div className="flex w-full justify-between items-center">
            ...
            <p className={`border rounded-xl px-41 py-2.5  text-xs tracking-widest self-start flex flex-col items-center uppercase font-jetbrains ${timer <= 7 && selected === null ? 'bg-[#1a0008] text-[rgba(255,51,102,0.6)] border-[#aa0033] animate-pulse' : timer <= 16 ? 'bg-[#140900] text-[rgba(255,149,0,0.6)] border-[#aa5500]' : 'bg-[#08081a] border-[#2a2a5a] text-[rgba(90,74,238,0.9)]'}`}>Time <span className={`font-bold lowercase text-2xl leading-tight ${timer <= 7 ? 'text-[#ff3366]' : timer <= 16 ? 'text-[#ff9500]' : 'text-[#9a8cee]'}`}>{timer}s</span></p>
